...
async def ativos(request: Request, db: Session = Depends(get_db)):
    lista = listar_ativos(db)
...
async def novo_ativo(
    ...
    cadastrar_ativo(db, nome, produto, versao)

    return RedirectResponse("/ativos", status_code=303)


@router.post("/ativos/editar/{id}")
async def editar(
    id: int,
    nome: str = Form(...),
    produto: str = Form(...),
    ...
    editar_ativo(db, id, nome, produto, versao)

    return RedirectResponse("/ativos", status_code=303)


@router.get("/ativos/excluir/{id}")
